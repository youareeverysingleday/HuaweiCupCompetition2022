...
2. 题目：
   1. “投放点数量的合理性分析，并做适当的优化”
      1. 多阶段优化对小区的特征进行抽象。决策变量是每个地点的投放数目。优化条件是设置一个上限和下限。优化条件是数量最少、服务的效用最大。边界的辐射等。
      2. 单目标优化。将站点数量作为约束进行求解。
   2. “考虑未来疫情、自然灾害等特殊事件，对于政府储备物资和大规模物资分拣场所的位置与数量规模进行合理规划，并提出最优的选址数量、规模及其潜在的备用场所位置。”
      1. 对疫情做出预测，以当前最坏情况来进行处理。
      2. 自然灾害在于“市州支援”、“市级直采”和“属地自保”三种来源的均衡分配。
      3. 对分拣场的优化-------
